...
def inference_model(model, filename, scenarionames, dataset_name, number, dataset, para, k=6, test=False, return_heatmap=True, mode='densetnt', batch=16):
    H = []
    Ua = []
    Ue = []
    Yp = []
    ...
    data = np.load('./interaction_merge/'+dataset_name+'/'+dataset_name+'_'+scenarionames+'.npz', allow_pickle=True)
    if return_heatmap:
        Na = data['nbagents']
        Nm = data['nbsplines']
        T = data['trajectory']
    ...
    nb = len(dataset)
    print("number of data: len(dataset)==", nb)
    ...
    for i in range(len(cut)-1):
        ...
        for j in range(number):
            model.encoder.load_state_dict(torch.load(filename+'encoder_'+scenarionames+'_'+ str(j) + '.pt'))
            model.decoder.load_state_dict(torch.load(filename+'decoder_'+scenarionames+'_'+ str(j) + '.pt'))
