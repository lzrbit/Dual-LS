...
def cal_buffer_loss_gss_logits(self):
    buf_inputs_past, buf_labels_past, buf_logits_past, buf_task_id_past = self.buffer_gss.get_data(self.args.minibatch_size, transform=self.transform, give_index=False)
...
def cal_buffer_loss_gss_positions(self):
    buf_inputs_past, buf_labels_past, buf_logits_past,  buf_task_id_past  = self.buffer_gss.get_data(self.args.minibatch_size, transform=self.transform, give_index=False)
    buf_outputs = self.net(buf_inputs_past)
